...
export const MatchGame = () => {
  ...
  const handleCardClick = (card) => {
    if (selected.length >= 2 || selected.find(s => s.id === card.id) || matched.includes(card.pairId)) {
      return;
    }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
          {cards.map((card) => {
            ...
            const isWrongMatch = selected.length === 2 && isSelected && !isMatched;

            return (
              <Card
                key={card.id}
                onClick={() => handleCardClick(card)}
                className={`h-32 cursor-pointer transition-all transform hover:scale-105 ${
                  isMatched 
                    ? 'bg-success/10 border-success opacity-50 cursor-not-allowed' 
                    : isSelected 
                    ? isWrongMatch 
              ...
              >
                <CardContent className="h-full flex items-center justify-center p-3">
                  <div className={`text-center ${
                    card.type === 'term' ? 'font-bold' : 'text-secondary'
                  }`}>
