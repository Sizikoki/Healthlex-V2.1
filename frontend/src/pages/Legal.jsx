...
export const Legal = ({ activeDoc: propActiveDoc }) => {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col justify-between text-foreground">
      ...
      {/* Sayfa Altı / Footer Bölümü */}
      <footer className="w-full bg-[#0F1E36] text-white py-10 mt-12 border-t border-white/10">
        <div className="max-w-[1140px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="text-center md:text-left">
              <Link to="/" className="inline-flex items-center gap-2">
                <div className="bg-primary p-1.5 rounded-md">
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M3 12h4l2 7 4-14 2 7h6" />
                  </svg>
                </div>
                <span className="text-lg font-bold tracking-tight text-white">
                  Health<span className="text-primary font-serif italic">Lex</span>Med
                </span>
              </Link>
              <p className="text-xs text-white/70 mt-1.5">
                {lang === 'tr'
                  ? 'Latince tıp terimlerini kök mantığıyla öğreten eğitim platformu.'
                  : 'Educational platform teaching Latin medical terminology through root logic.'}
              </p>
            </div>

            {/* İletişim & SLA Bilgisi */}
            <div className="text-center md:text-right text-xs text-white/80 space-y-1">
              ...
              <div className="text-white/60">
                {content.nav.responseSla}
              </div>
            </div>
          </div>

          {/* Hızlı Linkler & Yasal Sözleşmeler */}
          <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-white/60">
            <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6">
              ...
              <Link to="/morphemes" className="hover:text-white transition-colors">
                {t('morphemes', 'Morfemler')}
              </Link>
              <Link to="/contact" className="hover:text-white transition-colors">
                {content.nav.contactSupport}
              </Link>
              <Link
                to="/terms"
                className={`transition-colors ${activeKey === 'terms' ? 'text-white font-bold underline' : 'hover:text-white'}`}
              >
                {content.nav.terms}
              </Link>
              <Link
                to="/privacy"
                className={`transition-colors ${activeKey === 'privacy' ? 'text-white font-bold underline' : 'hover:text-white'}`}
              >
                {content.nav.privacy}
              </Link>
              <Link
                to="/refund"
                className={`transition-colors ${activeKey === 'refund' ? 'text-white font-bold underline' : 'hover:text-white'}`}
              >
                {content.nav.refund}
              </Link>
            </div>
            ...
            <div className="text-white/50 text-[11px]">
              © 2026 HealthLexMed · healthlexmed.com
            </div>
          </div>
        </div>
      </footer>
