    ...
    <section id="s8" className="flex flex-col gap-[22px]">
      ...
      <div className="grid grid-cols-2 gap-[14px]">
        {STUDY_METHODS.map((item) => (
          <div
            key={item.num}
            className="bg-white border border-[#e3e8f1] rounded-[16px] p-[22px] flex flex-col gap-[8px]"
          >
            <span className="font-['Lora',serif] font-bold text-[22px] leading-none text-[#1d4ed8]">
              {item.num}
            </span>
      ...
      <div className="mt-[18px] bg-[#0f1b33] rounded-[20px] p-[36px] flex flex-col gap-[24px]">
        <div className="flex flex-col gap-[8px]">
          <span className="font-extrabold text-[12px] leading-none uppercase tracking-[0.14em] text-[#9fb4d6]">
            HealthLexMed&apos;de uygula
          </span>
          <h3 className="m-0 font-['Lora',serif] font-bold text-[26px] leading-[1.25] text-white">
        ...
        <div className="grid grid-cols-3 gap-[12px]">
          {HEALTHLEX_APPS.map((app) => (
            <Link
              key={app.title}
              to={app.path}
              className="bg-[#182a4a] border border-[#2a3f63] rounded-[14px] p-[18px] flex flex-col gap-[6px] hover:bg-[#1f375f] hover:border-[#3b5788] transition-all"
            >
        ...
        <Link
          to="/morphemes"
          className="self-start bg-white text-[#0f1b33] font-extrabold text-[15px] px-[22px] py-[14px] rounded-[11px] hover:bg-[#f1f5f9] transition-colors"
        >
