...
export const Profile = () => {
  ...
  const handleOpenPortal = async () => {
    try {
      ...
      const targetUrl = isLifetime ? (data.overviewUrl || data.url) : data.url;
                ...
                <div className="max-w-xl">
                  ...
                  <p className="text-[0.88rem] text-[var(--muted)] leading-relaxed m-0">
                    {isLifetime
                      ...
                      ? (isBasic
                          ? (isTr
                              ? '3 günlük ücretsiz deneme sürecindesiniz. Tüm 16 kategori, 571+ morfem ve 4 oyun modunun tamamına sınırsız erişebilirsiniz.'
                              : 'You are on a 3-day free trial. Full access to all 16 categories, 571+ morphemes, and 4 game modes.')
                          : (isTr
                              ? '3 günlük ücretsiz deneme sürecindesiniz. 16 kategorinin tamamı, 571+ morfem ve 4 oyun moduna sınırsız erişebilirsiniz.'
                              : 'You are on a 3-day free trial. Full access to all 16 categories, 571+ morphemes, and 4 game modes.'))
                      : isProPlan
                      ? (isTr
                      ...
                      : isBasic
                      ? (isTr
                          ? 'Temel plandasınız: 16 kategori sözlüğü, 100 morfem ve sınırsız Flashcard & Eşleştirme açık. Tüm 571+ morfem ve Quiz için Pro\'ya geçebilirsiniz.'
                          : 'You are on the Basic plan: 16 categories glossary, 100 morphemes, and unlimited Flashcards & Matching unlocked. Upgrade to Pro for all 571+ morphemes and Quiz.')
                      : (isTr
                  ...
                  {trialState.isActive && (
                    <div className="mt-3 text-xs font-semibold text-amber-700 dark:text-amber-400 bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-lg inline-flex items-center gap-2">
                      <span>⏳</span>
                  ...
                  {isLifetime && (
                    ...
                    >
                      ...
                      {portalLoading
                        ...
                        : (isTr ? 'Faturalarım / Makbuz' : 'Invoices / Receipts')}
                    </button>
                  )}
              ...
              <div className="setting-row flex justify-between items-center gap-[16px] py-[16px] last:border-none">
                <div>
                  ...
                  <div className="setting-sub text-[0.8rem] text-[var(--muted)] mt-0.5 max-w-[420px]">
                    {isTr 
                      ? '16 kategori, öğrenilen terimler, rozetler ve çalışma istatistiklerinizi içeren resmi PDF raporu.' 
                      : 'Official PDF report containing 16 categories, learned terms, badges, and study stats.'}
                  </div>
                  <button
                    type="button"
