...
const DATA_FILE_PATH = path.join(__dirname, "../data_archive/kalan_maddeler.json");
...
async function run() {
  ...
  if (initialMaxId !== 810) {
    throw new Error(`Safety abort: initial max ID is ${initialMaxId}, expected 810!`);
  }
  ...
  // Fetch requested documents: 811, 819, 828
  const checkDocIds = ['811', '819', '828'];
  const fetchedDocs = {};
