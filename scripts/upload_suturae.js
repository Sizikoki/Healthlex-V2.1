...
const DATA_FILE_PATH = path.join(__dirname, "../data_archive/suturae.json");
...
async function run() {
  ...
  if (initialMaxId !== 782) {
    throw new Error(`Safety abort: initial max ID is ${initialMaxId}, expected 782!`);
  }
  ...
  // Fetch requested documents: 783, 796, 810
  const checkDocIds = ['783', '796', '810'];
  const fetchedDocs = {};
